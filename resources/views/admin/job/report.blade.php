...
<div id="content" class="container-fluid">
    @forelse($families as $key=>$family)
        @php
            $point = 0;
            if (isset($family->member))
            {
                foreach ($family->member as $val)
                {
                    if ($val->user)
                    {
        ...
        <div class="card-body">
            ...
            @forelse($family->member as $key => $member)
                <div class="form-group row">
                    ...
                    <label class="col-sm-2" for="namefamily">{{$member->user->first_name ?? 'N/A'}}</label>
                    <label class="col-sm-2" for="namefamily">{{$member->user->point ?? '0'}}</label>
                </div>
            @empty
            @endforelse
        </div>
